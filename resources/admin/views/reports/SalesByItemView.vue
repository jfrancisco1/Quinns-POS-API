...
const itemSummary = computed(() => {
  ...
  filteredOrders.value.forEach(order => {
    ...
    items.forEach(item => {
      const label = item.label || item.item?.name || 'Unknown'
      if (!map[label]) map[label] = { units: 0, revenue: 0 }
      map[label].units += parseInt(item.quantity || item.qty || 1)
      map[label].revenue += parseFloat(item.subtotal || (parseFloat(item.price || 0) * parseInt(item.quantity || item.qty || 1)))
    })
  })
  return Object.entries(map).map(([label, d]) => ({ label, ...d })).sort((a, b) => b.revenue - a.revenue)
})

const chartData = computed(() => ({
  labels: itemSummary.value.map(i => i.label),
  datasets: [
    { label: 'Revenue (₱)', data: itemSummary.value.map(i => i.revenue), backgroundColor: 'rgba(59,130,246,0.7)' },
  ]
}))
...
<template>
  <div>
    <div class="mb-6">
      ...
      <p class="text-sm text-gray-500 mt-1">Revenue breakdown per service item</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <h2 class="font-semibold text-gray-900 mb-4">Revenue by Item</h2>
      ...
      <div class="bg-white rounded-xl border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-100">
          <h2 class="font-semibold text-gray-900">Item Summary</h2>
        </div>
        <div v-if="!itemSummary.length" class="px-6 py-8 text-center text-sm text-gray-400">No data.</div>
        <table v-else class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="item in itemSummary" :key="item.label" class="hover:bg-gray-50">
              <td class="px-6 py-3 font-medium text-gray-900">{{ item.label }}</td>
              <td class="px-6 py-3 text-gray-600">{{ item.units }}</td>
              <td class="px-6 py-3 font-medium text-gray-900">{{ fmt(item.revenue) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
