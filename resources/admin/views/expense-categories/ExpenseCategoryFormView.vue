...
import { getExpenseCategory, createExpenseCategory, updateExpenseCategory } from '../../api'
...
onMounted(async () => {
  if (isEdit.value) {
    try {
      const { data } = await getExpenseCategory(route.params.id)
      const cat = data.data
      form.value = {
        name: cat.name || '',
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateExpenseCategory(route.params.id, form.value)
    } else {
      await createExpenseCategory(form.value)
    }
...
<template>
  <div>
    <div class="mb-6 flex items-center gap-4">
      <button @click="router.push('/expense-categories')" class="text-gray-500 hover:text-gray-700 transition-colors">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
      </button>
      <div>
        <h1 class="text-2xl font-bold text-gray-900">{{ isEdit ? 'Edit Expense Category' : 'New Expense Category' }}</h1>
        <p class="text-sm text-gray-500 mt-0.5">{{ isEdit ? 'Update category details' : 'Add a new expense category' }}</p>
      </div>
    </div>
    ...
    <div v-else class="bg-white rounded-xl border border-gray-200 p-6 max-w-lg">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div v-if="isEdit">
          ...
          <p class="text-xs text-gray-500 mt-1 ml-7">Inactive categories won't be offered when recording a new expense.</p>
        </div>
        ...
        <div class="flex gap-3 pt-2">
          ...
          <button type="button" @click="router.push('/expense-categories')" class="px-5 py-2.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors">
            Cancel
          </button>
        </div>
      </form>
